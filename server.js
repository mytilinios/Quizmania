...
app.get("/api/deezer-tracks", async (req, res) => {
  ...
  // Map our categories to Deezer genre search terms
  const genreMap = {
    'laika': 'λαϊκά', 'palia_laika': 'παλιά λαϊκά', 'ellhnikh_pop': 'ελληνική pop',
    'entexna': 'έντεχνα', 'rok': 'ελληνικό ροκ', 'hiphop': 'ελληνικό hip hop',
    'pop': 'pop hits', 'mixta': 'ελληνικά'
  };
  
  const searchTerm = genreMap[genre] || genre || 'ελληνικά';
  
  return new Promise((resolve) => {
    const q = encodeURIComponent(searchTerm);
    const opts = { hostname: 'api.deezer.com', path: `/search?q=${q}&limit=50`, method: 'GET' };
    https.get(opts, apiRes => {
      ...
      apiRes.on('end', () => {
        try {
          ...
          const withPreview = (parsed.data || []).filter(t => t.preview && t.title && t.artist?.name);
          // Shuffle and pick n
          ...
          const selected = withPreview.slice(0, n * 2); // Get double, AI will pick best
          const tracks = selected.map(t => ({ title: t.title, artist: t.artist.name, preview: t.preview }));
          res.json({ tracks });
        } catch(e) { res.json({ tracks: [] }); }
      });
    }).on('error', () => res.json({ tracks: [] }));
    resolve();
  });
});
