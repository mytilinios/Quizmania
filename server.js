...
app.get("/api/preview", async (req, res) => {
  ...
  try {
    const previewUrl = await searchDeezerPreview(track, artist || "");
    res.json({ previewUrl });
  } catch(e) {
...
// ── Deezer Genre Tracks (get real tracks with previews by genre) ───────
app.get("/api/deezer-tracks", async (req, res) => {
  const { genre, count } = req.query;
  const n = parseInt(count) || 10;
  
  // Multiple search terms per genre for better coverage
  const genreMap = {
    'laika':        ['Θανάσης Πολυκανδριώτης', 'Γιώργος Νταλάρας λαϊκά', 'Στράτος Διονυσίου', 'Βασίλης Καρράς', 'Πάνος Κιάμος'],
    'palia_laika':  ['Στέλιος Καζαντζίδης', 'Γρηγόρης Μπιθικώτσης', 'Μαρινέλλα παλιά', 'Χαρούλα Αλεξίου παλιά', 'Πολυξένη Βασιλείου'],
    'ellhnikh_pop': ['Δέσποινα Βανδή', 'Αντώνης Ρέμος', 'Νότης Σφακιανάκης', 'Ελένη Φουρέιρα', 'Πάνος Μουζουράκης'],
    'entexna':      ['Χάρις Αλεξίου έντεχνα', 'Γιώργος Νταλάρας έντεχνα', 'Αλκίνοος Ιωαννίδης', 'Σαββόπουλος', 'Μίλτος Πασχαλίδης'],
    'rok':          ['Ξυλούρης ροκ', 'Τρύπες', 'Νέγρος του Μοριά', 'Σωκράτης Μάλαμας', 'Χρήστος Θηβαίος'],
    'hiphop':       ['Παυλάκης', 'Light', 'Bariş Manço', 'Ελληνικό hip hop', 'Greek rap'],
    'pop':          ['Ed Sheeran', 'Adele', 'Taylor Swift', 'The Weeknd', 'Dua Lipa'],
    'mixta':        ['Δέσποινα Βανδή', 'Αντώνης Ρέμος', 'Χάρις Αλεξίου', 'Γιώργος Νταλάρας', 'Νότης Σφακιανάκης']
  };
  
  const searchTerms = genreMap[genre] || genreMap['mixta'];
  
  function searchDeezer(term) {
    return new Promise((resolve) => {
      const q = encodeURIComponent(term);
      const opts = { hostname: 'api.deezer.com', path: `/search?q=${q}&limit=20`, method: 'GET' };
        ...
        apiRes.on('end', () => {
          try {
            ...
            const tracks = (parsed.data || [])
              ...
              .map(t => ({ title: t.title, artist: t.artist.name, preview: t.preview }));
            resolve(tracks);
          } catch(e) { resolve([]); }
        });
      }).on('error', () => resolve([]));
  ...
  try {
    ...
    const results = await Promise.all(searchTerms.map(t => searchDeezer(t)));
    let allTracks = [];
    results.forEach(r => allTracks = allTracks.concat(r));
    ...
    console.log(`Deezer: found ${unique.length} unique tracks for genre ${genre}`);
    res.json({ tracks: unique.slice(0, n * 3) });
  } catch(e) {
    res.json({ tracks: [] });
  }
});
